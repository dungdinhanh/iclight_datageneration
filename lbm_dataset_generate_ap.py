...
import argparse
def main():
    parser = argparse.ArgumentParser(description="IC-Light Batch Processor")
    ...
    parser.add_argument("--source_folder", type=str, required=True, help="Folder to save output images")
    parser.add_argument("--background_dir", type=str, required=True, help="Directory containing background images")
    parser.add_argument("--image_width", type=int, default=480, help="Target image width")
    parser.add_argument("--image_height", type=int, default=640, help="Target image height")
    ...
    def process_batch(
        ...
        fgs = resize_and_center_crop(input_fgs, new_width, new_height)
        processed_bgs = resize_and_center_crop(input_bgs, new_width, new_height)
    ...
    def load_image_and_mask_paths_from_metadata(
        ...
        image_paths = [os.path.join(image_folder, os.path.basename(fname)) for fname in df['image_path']]
        mask_paths = [os.path.join(mask_folder, os.path.basename(fname)) for fname in df['mask_path']]
    ...
    background_samples =  sorted(
                                    glob.glob(os.path.join(args.background_dir, "*.[jp][pn]g"))
                                ) 
